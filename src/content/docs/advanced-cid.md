---
title: 'Advanced CID API'
description: 'Content-addressed storage: CID extraction, verification, and public download by CID.'
---
...
> The Advanced CID API is exported separately (`@julesl23/s5js/advanced`) and does not affect the simplicity of the standard path-based API.
...
import { FS5Advanced, formatCID, parseCID, verifyCID } from '@julesl23/s5js/advanced';
```
...
> **Note (beta.50+):** if a directory in the search is *temporarily unavailable* (transient network 404), `cidToPath()` throws a retryable `S5DirectoryLoadError` instead of returning a false `null` ("CID not found"). Retry with backoff — see [Error Handling](/path-api/#error-handling). `null` now reliably means the CID genuinely isn't referenced anywhere in your tree.
...
### Public Sharing Between Users

The `downloadByCID` method enables true public sharing - one user uploads content, shares the CID, and another user downloads it from S5 portals:

```typescript
// === User A: Upload and share ===
const s5a = await S5.create();
await s5a.recoverIdentityFromSeedPhrase(seedPhraseA);
...
const s5b = await S5.create();
await s5b.recoverIdentityFromSeedPhrase(seedPhraseB);
await s5b.registerOnNewPortal('https://s5.ninja');

// Download using the shared CID
const downloadedData = await s5b.downloadByCID(cidString);
console.log('Downloaded:', downloadedData.length, 'bytes');
```

**How it works:**
1. User A uploads content (stored on S5 network via portal)
2. User A extracts the CID (32-byte BLAKE3 hash)
3. User A shares the CID string (53 or 59 characters)
4. User B downloads from any configured portal using the CID
5. Downloaded data is verified against the CID hash

**CID Formats Supported:**
- 53-char format: Raw 32-byte BLAKE3 hash (`b` + 52 base32 chars)
- 59-char format: BlobIdentifier with prefix + hash + size
...
- **[Path-based API](/path-api/)** - Standard file operations
- **[Cross-Identity Public Directory Access](/cross-identity/)** - Multi-user reads and live subscriptions using shared public keys (a different kind of "public content" than CID-addressed blobs)
- **[Performance & Scaling](/performance/)** - Optimize large datasets
- **[API Reference](/api-reference/)** - Complete API documentation
- **[S5 CID Specification](https://docs.sfive.net/spec/blobs.html)** - CID format details
