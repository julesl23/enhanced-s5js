...
description: 'Recursive directory traversal and batch copy/delete operations with progress tracking.'
---
...
### Transient Unavailability Is Never Silently Skipped (beta.50+)

If a directory in the tree is *temporarily unavailable* (its registry entry exists but the blob can't be downloaded right now — for example, a transient 404 while the network propagates), `DirectoryWalker.walk()`, `copyDirectory()`, and `deleteDirectory()` **re-throw** the retryable `S5DirectoryLoadError` instead of skipping the subtree. This applies even with `onError: 'continue'`, which only skips *genuine* per-item failures.

Without this, a transient outage could make a copy or delete silently drop an entire subtree while still reporting success. Catch the error and retry the whole operation:

```typescript
import { isS5DirectoryLoadError } from '@julesl23/s5js';

try {
  await batch.copyDirectory('home/source', 'archive/dest', { onError: 'continue' });
} catch (error) {
  if (isS5DirectoryLoadError(error) && error.retryable) {
    // A directory was temporarily unavailable — retry the copy after a backoff.
  } else {
    throw error;
  }
}
...
- **[Performance](/performance/)** - Optimize for large directories
