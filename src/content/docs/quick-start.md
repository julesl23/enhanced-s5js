---
title: 'Quick Start'
description: 'From npm install to your first file stored on S5 in five minutes.'
---
...
### "S5DirectoryLoadError" Thrown

A `S5DirectoryLoadError` with `retryable: true` means a directory is known to exist but its content is temporarily unavailable (e.g., a transient 404 while the network propagates). Retry the operation with backoff — don't treat it as a missing file. See [Error Handling](/path-api/#error-handling).

## Further Reading

- **[Path-based API Guide](/path-api/)** - Complete API documentation
- **[Identity & Signing API](/identity/)** - Backend-mediated portal registration
- **[Cross-Identity Public Directory Access](/cross-identity/)** - Read another user's data and live-subscribe to updates
- **[Media Processing](/media/)** - Image thumbnails and metadata
- **[Performance & Scaling](/performance/)** - HAMT for large directories
- **[Encryption](/encryption/)** - Secure your data
- **[Installation & Setup](/installation/#debug-logging)** - Enable debug logging (`DEBUG=s5js:*`) for diagnostics
